...
using alternate_from_mem_type =
  std::variant_alternative_t<variant_index_from_memory_type(MemType), Variant>;

namespace detail {

template <typename DstElementType,
          typename DstExtents,
          ...
          std::enable_if_t<std::conjunction_v<std::is_convertible_v<SrcElementType, DstElementType>,
                                              SrcExtents::rank() == DstExtents::rank()>>* = nullptr>
void copy(resources const& res,
          ...
          ExecutionPolicy host_exec_policy = std::execution::unseq)
{
  ...
  if constexpr (
    ...
    std::conjunction_v<std::is_same_v<DstLayoutPolicy, SrcLayoutPolicy>,
                       std::disjunction_v<std::is_same_v<DstLayoutPolicy, layout_c_contiguous>,
                                          std::is_same_v<DstLayoutPolicy, layout_f_contiguous>>>) {
    if constexpr (
      std::
        disjunction_v<std::conjunction_v<CUDA_ENABLED, !DstAccessorPolicy::mem_type::is_device_accessible, !SrcAccessorPolicy::mem_type::is_device_accessible>, std::conjunction_v<!CUDA_ENABLED, DstAccessorPolicy::mem_type::is_host_accessible, SrcAccessorPolicy::mem_type::is_host_accessible>, >) {
      std::copy(
        host_exec_policy, src.data_handle(), src.data_handle() + src.size(), dst.data_handle());
    } else {
#ifndef RAFT_DISABLE_CUDA
...
#else
      throw non_cuda_build_error{"Attempted copy to/from device in non-CUDA build"};
#endif
    }
  } else {  // Non-contiguous memory or transpose required
    if constexpr (std::conjunction_v<DstAccessorPolicy::mem_type::is_device_accessible,
                                     SrcAccessorPolicy::mem_type::is_device_accessible>) {
      // TODO(wphicks): Conversion/transpose kernel
    } else if constexpr (std::conjunction_v<DstAccessorPolicy::mem_type::is_host_accessible,
...
struct mdbuffer {
  ...
  template <typename AccessorPolicy,
            std::enable_if_t<
              std::is_convertible_v<mdspan<ElementType, Extents, LayoutPolicy, AccessorPolicy>,
                                    storage_type_variant>>* = nullptr>
  ...
  template <typename OtherContainerPolicy,
            std::enable_if_t<std::is_convertible_v<
              typename mdarray<ElementType, Extents, LayoutPolicy, OtherContainerPolicy>::view_type,
              storage_type_variant>>* = nullptr>
  ...
  template <typename OtherContainerPolicy,
            std::enable_if_t<std::is_convertible_v<
              mdarray<ElementType, Extents, LayoutPolicy, OtherContainerPolicy>,
              storage_type_variant>>* = nullptr>
  constexpr mdbuffer(mdarray<ElementType, Extents, LayoutPolicy, OtherContainerPolicy>&& other)
    : data_{std::move(other)}
  {
  ...
  template <
    ...
    std::enable_if_t<std::conjunction_v<std::is_convertible_v<ElementType, OtherElementType>,
                                        Extents::rank() == OtherExtents::rank()>>* = nullptr>
  constexpr mdbuffer(
    resources const& res,
